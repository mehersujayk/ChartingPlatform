...
def drawUI(app, canvas):
    ...
    canvas.create_text(app.graphLeft + (w/16), h - (h/20), text = 'New Stock')
    ...
    #move right
    canvas.create_rectangle(app.graphRight - (w/4), h - (h/15), app.graphRight - (w/4) + (w/64),  h - (h/30), fill = 'red')

    #move left
    canvas.create_rectangle(app.graphLeft + (w/4), h - (h/15), app.graphLeft + (w/4) + (w/64), h - (h/30), fill = 'green')
...
def Buy_drawBuyUI(app, canvas):
    ...
    #exchange
    canvas.create_text(w/12, (5*h)/8, text='Exchange', font='Arial 18 bold', anchor='w')
    canvas.create_text(w - (w/10), (5*h)/8, text=f'{app.exchange}', anchor='e')
...
def Sell_drawBuyUI(app, canvas):
    ...
    #exchange
    canvas.create_text(w/9, (2*h)/3, text='Exchange', font='Arial 18 bold')
    canvas.create_text(w - (w/6), (2*h)/3, text=f'{app.exchange}')
...
def drawAnalyticsUI(app, canvas):
    ...
    actualReturn = totalReturn / len(app.buys)
